...
export class TaskService {
  ...
  getAllGroupTask(id: number): Observable<ApiResponse<Task[]>> {
    return this.http.get<ApiResponse<Task[]>>(`${this.apiUrl}/group/${id}`)
      .pipe(catchError(this.handleError));
  }

  getRelatedTasksByTaskId(id: number): Observable<ApiResponse<Task[]>> {
    return this.http.get<ApiResponse<Task[]>>(`${this.apiUrl}/${id}/relations`)
      .pipe(catchError(this.handleError));
  }
  //Posts
  ...
  //UpdatesTask
  updateTask(taskId: number, task: Task): Observable<ApiResponse<Task>> {
    return this.http.put<ApiResponse<Task>>(`${this.apiUrl}/${taskId}`, task)
  }
